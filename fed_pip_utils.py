...
def FedWeightedAvg(server, models, clients_weight, distributed):
    ...
    with torch.no_grad():
        if distributed:
            server.g_prompt.data = models[0].module.g_prompt.data*clients_weight[0]
        else:
            server.g_prompt.data = models[0].g_prompt.data*clients_weight[0]

        total_weight += clients_weight[0]

        for i in range(1, len(models)):
            if distributed:
                server.g_prompt.data += models[i].module.g_prompt.data*clients_weight[i]
            else:
                server.g_prompt.data += models[i].g_prompt.data*clients_weight[i]
            
            total_weight += clients_weight[i]

        server.g_prompt.data = torch.div(server.g_prompt.data, total_weight).clone()
    ...
    with torch.no_grad():
        if distributed:
            server.e_prompt.prompt_key.data = models[0].module.e_prompt.prompt_key.data*clients_weight[0]
            server.e_prompt.prompt.data = models[0].module.e_prompt.prompt.data*clients_weight[0]
        else:
            server.e_prompt.prompt_key.data = models[0].e_prompt.prompt_key.data*clients_weight[0]
            server.e_prompt.prompt.data = models[0].e_prompt.prompt.data*clients_weight[0]


        for i in range(1, len(models)):
            if distributed:
                server.e_prompt.prompt_key.data += models[i].module.e_prompt.prompt_key.data*clients_weight[i]
                server.e_prompt.prompt.data += models[i].module.e_prompt.prompt.data*clients_weight[i]
            else:
                server.e_prompt.prompt_key.data += models[i].e_prompt.prompt_key.data*clients_weight[i]
                server.e_prompt.prompt.data += models[i].e_prompt.prompt.data*clients_weight[i]

        server.e_prompt.prompt.data = torch.div(server.e_prompt.prompt.data , total_weight).clone()
        server.e_prompt.prompt_key.data = torch.div(server.e_prompt.prompt_key.data , total_weight).clone()
...
def FedWeightedAvgWithHead(server, models, clients_weight, distributed):
    ...
    with torch.no_grad():
        if distributed:
            server.head.weight.data = models[0].module.head.weight.data*clients_weight[0]
            server.head.bias.data = models[0].module.head.bias.data*clients_weight[0]
        else:
            server.head.weight.data = models[0].head.weight.data*clients_weight[0]
            server.head.bias.data = models[0].head.bias.data*clients_weight[0]

        total_weight += clients_weight[0]

        for i in range(1, len(models)):
            if distributed:
                server.head.weight.data += models[i].module.head.weight.data*clients_weight[i]
                server.head.bias.data += models[i].module.head.bias.data*clients_weight[i]
            else:
                server.head.weight.data += models[i].head.weight.data*clients_weight[i]
                server.head.bias.data += models[i].head.bias.data*clients_weight[i]

            total_weight += clients_weight[i]

        server.head.weight.data = torch.div(server.head.weight, total_weight).clone()
        server.head.bias.data = torch.div(server.head.bias, total_weight).clone()
    print("Agregating head done")
    ...
    with torch.no_grad():
        ...
        for i in range(1, len(models)):
            if distributed:
                ...
                server.g_prompt.data += models[i].g_prompt.data*clients_weight[i]
        # model_agg.g_prompt = torch.nn.Parameter(torch.div(model_agg.g_prompt, len(models)))
        server.g_prompt.data = torch.div(server.g_prompt.data, total_weight)
    print("Agregating g_prompt done")
...
def FedWeightedAvgPrototype(clients_prototype, clients_prototype_var, clients_weight, task_id, classes_per_task):
